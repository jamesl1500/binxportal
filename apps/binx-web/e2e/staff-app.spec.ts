...
test.describe("staff app", () => {
  ...
  test("creating a lead shows the AI follow-up card", async ({ staffPage }) => {
    ...
    // The create page lands straight on the new lead's own page.
    await expect(staffPage).toHaveURL(/\/leads\/[0-9a-f-]{36}$/);
    await expect(staffPage.getByRole("heading", { level: 1, name: "Riverside Outfitters" })).toBeVisible();
